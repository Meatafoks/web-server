...
export async function attachControllers(app: Express | Router, controllers: Type[]) {
  // @ts-ignore
  const promises = controllers.map((controller: Type) => registerController(app, controller, getController))
...
async function getController(Controller: Type): Promise<ExpressClass> {
  try {
    if (!WebServerContainer.has(Controller)) {
    ...
    return await WebServerContainer.get(Controller)
  } catch {
